...
class MetricsCollector:
    ...
    def record_task_start(self, task_id: str, worker_id: str) -> None:
        ...
        self.tasks[task_id] = TaskMetrics(
            task_id=task_id, worker_id=worker_id, started_at=datetime.now()
        )
    ...
    def get_summary(self) -> dict:
        ...
        return {
            ...
            "completed_tasks": sum(
                1 for t in self.tasks.values() if t.status == "complete"
            ),
            "failed_tasks": sum(
            ...
            "levels": [
                {
                    ...
                    "completed_at": (
                        lv.completed_at.isoformat() if lv.completed_at else None
                    ),
