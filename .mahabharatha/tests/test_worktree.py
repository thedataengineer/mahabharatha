...
class TestWorktreeMerge:
    ...
    def test_merge_no_conflicts(self, git_repo):
        ...
        subprocess.run(["git", "commit", "-m", "Add test"], cwd=wt.path, capture_output=True)
...
class TestLevelMerge:
    ...
    def test_merge_level_branches(self, git_repo):
        ...
        subprocess.run(["git", "commit", "-m", "W1 work"], cwd=wt1.path, capture_output=True)

        (wt2.path / "file2.txt").write_text("from w2")
        subprocess.run(["git", "add", "."], cwd=wt2.path, capture_output=True)
        subprocess.run(["git", "commit", "-m", "W2 work"], cwd=wt2.path, capture_output=True)
...
class TestGitIntegration:
    ...
    def test_git_worktree_list(self, git_repo):
        ...
        result = subprocess.run(["git", "worktree", "list"], cwd=git_repo, capture_output=True, text=True)
    ...
    def test_branch_created_from_base(self, git_repo):
        ...
        result = subprocess.run(["git", "branch", "-l"], cwd=git_repo, capture_output=True, text=True)
