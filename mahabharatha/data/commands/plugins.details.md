...
**Discovery**: `PluginRegistry.load_entry_points("mahabharatha.plugins")` runs on orchestrator startup

## Security Model

Mahabharatha plugins are **strictly additive** and cannot compromise orchestrator integrity:
...
```python
# mahabharatha/plugins.py:119-134
...
```python
# mahabharatha/plugins.py:189-190
...
class K8sLauncher(WorkerLauncher):
    def __init__(self, mahabharatha_config):
        super().__init__(mahabharatha_config)
    ...
    def cleanup(self, launch_info: dict) -> None:
        self.api.delete_namespaced_pod(name=launch_info["pod_name"], namespace="mahabharatha")
...
**Fixes**:
1. Check entry point group name: `[project.entry-points."mahabharatha.plugins"]`
2. Reinstall package: `pip install -e .`
3. Verify logs: `.mahabharatha/logs/orchestrator.log` shows "Registered {type} plugin: {name}"
...
4. Review logs: `.mahabharatha/logs/orchestrator.log` for full traceback
...
**Fixes**:
1. Test gate command manually: `cd .mahabharatha/merged && <gate_command>`
2. Check timeout — gates default to 5 minutes
3. Verify `GateRunResult` return value — must include all fields
4. Review logs: `.mahabharatha/logs/gates/level-{N}.log`
...
**Fixes**:
1. Check launcher name matches config: `mahabharatha_config.launcher.mode`
2. Verify `create_launcher()` returns `WorkerLauncher` subclass
3. Test plugin discovery: `python -c "from mahabharatha.plugins import PluginRegistry; r = PluginRegistry(); r.load_entry_points(); print(r._launchers)"`
...
- **Plugin ABCs**: `mahabharatha/plugins.py:51-87`
- **Plugin Registry**: `mahabharatha/plugins.py:95-242`
- **Config Models**: `mahabharatha/plugin_config.py:6-37`
- **Lifecycle Events**: `mahabharatha/constants.py:143-149`
