...
```bash
STAGING_BRANCH="mahabharatha/$FEATURE/staging-level-$CURRENT_LEVEL"
...
for branch in $BRANCHES; do
  ...
  if git merge --no-ff "$branch" -m "Merge $branch into staging"; then
    ...
    git diff --name-only --diff-filter=U > ".mahabharatha/conflicts-level-$CURRENT_LEVEL.txt"
    if [ "$FORCE" != "true" ]; then
...
cat .mahabharatha/conflicts-level-N.txt
...
mahabharatha retry TASK-ID --on-base
```
...
├──────────┼──────────────────────────────┼──────────┼──────────┤
│ worker-0 │ mahabharatha/{feature}/worker-0      │ 3        │ Ready    │
│ worker-1 │ mahabharatha/{feature}/worker-1      │ 2        │ Ready    │
│ worker-2 │ mahabharatha/{feature}/worker-2      │ 4        │ Ready    │
└──────────┴──────────────────────────────┴──────────┴──────────┘
...
Tag: mahabharatha/{feature}/level-2-complete

Next: Level 3 tasks are now unblocked.
Run /mahabharatha:status to see progress.
═══════════════════════════════════════════════════════════════
```
...
- Check for blocked/failed tasks with `/mahabharatha:status`
- Use `--force` only if you're sure partial merge is acceptable

### "Merge conflict detected"
- Review conflicting files in `.mahabharatha/conflicts-level-N.txt`
