...
services:
  worker:
    ...
    environment:
      - MAHABHARATHA_WORKER_ID=${{WORKER_ID:-0}}
...
class DevcontainerGenerator:
    ...
    def _generate_post_create(self, language: str) -> None:
        ...
        if language == "python":
            install_commands.append("if [ -f requirements.txt ]; then pip install -r requirements.txt; fi")
            install_commands.append("if [ -f pyproject.toml ]; then pip install -e .; fi")
        elif language == "typescript":
