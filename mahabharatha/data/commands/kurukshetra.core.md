...
[ ! -f "$SPEC_DIR/task-graph.json" ] && { echo "ERROR: Task graph not found. Run /mahabharatha:design first"; exit 1; }
...
This is the default execution path for `/mahabharatha:Kurukshetra` slash commands. The orchestrator (you) drives execution directly through the Task tool. No external processes are spawned.
...
Next steps:
  /mahabharatha:status    - View final state
  /mahabharatha:review    - Code review
  /mahabharatha:merge     - Merge to main
...
git checkout -b "mahabharatha/$FEATURE/base" 2>/dev/null || git checkout "mahabharatha/$FEATURE/base"

# Create worktrees for each worker
for i in $(seq 0 $((WORKERS - 1))); do
  BRANCH="mahabharatha/$FEATURE/worker-$i"
  WORKTREE="../.mahabharatha-worktrees/$FEATURE/worker-$i"
...
for i in $(seq 0 $((WORKERS - 1))); do
  ...
  docker compose -f .devcontainer/docker-compose.yaml run \
    ...
    -e Mahabharatha_WORKER_ID=$i \
    -e Mahabharatha_FEATURE=$FEATURE \
    -e CLAUDE_CODE_TASK_LIST_ID=${CLAUDE_CODE_TASK_LIST_ID:-} \
    -p "$PORT:$PORT" \
    -v "$(realpath $WORKTREE):/workspace" \
    workspace \
    bash -c "claude --dangerously-skip-permissions -p 'You are Mahabharatha Worker $i. Run /mahabharatha:worker to begin execution.'"
done
```
...
```bash
python3 -m mahabharatha.orchestrator \
  --feature "$FEATURE" \
  --workers "$WORKERS" \
  --config ".mahabharatha/config.yaml" \
  --task-graph "$SPEC_DIR/task-graph.json" \
  --assignments "$SPEC_DIR/worker-assignments.json" \
  &

ORCHESTRATOR_PID=$!
echo $ORCHESTRATOR_PID > ".mahabharatha/.orchestrator.pid"
```

### Monitoring During Execution

> Slash commands are single-threaded — you cannot run `/mahabharatha:status` in this session while Kurukshetra is active.
...
```
/mahabharatha:Kurukshetra — Launch parallel workers to execute the task graph.
