...
- Create, modify, or delete source code files (anything in `mahabharatha/`, `tests/`, `src/`, etc.)
...
```bash
FEATURE=${Mahabharatha_FEATURE:-"$ARGUMENTS"}

# Validate feature name
if [ -z "$FEATURE" ]; then
  echo "ERROR: Feature name required"
  echo "Usage: /mahabharatha:plan feature-name"
  exit 1
fi
...
Call TaskCreate:
  ...
  - description: "Planning phase for {feature}. Gathering requirements via /mahabharatha:plan."
...
After user approves requirements, call TaskUpdate to mark completed:
  ...
  - status: "completed"
  - description: "Requirements captured and approved for {feature}. Ready for /mahabharatha:design."
