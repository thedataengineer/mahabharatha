...
[ ! -f "$STATE_FILE" ] && { echo "ERROR: No state file found. Run /mahabharatha:Kurukshetra first"; exit 1; }
```
...
```bash
MAX_RETRIES=$(jq '.workers.retry_attempts' .mahabharatha/config.yaml)
...
if [ -n "$WORKER" ]; then
  ...
  python -m mahabharatha.orchestrator retry-task --task TASK-ID --feature "$FEATURE"
fi
```
...
Feature: {feature}
State: .mahabharatha/state/{feature}.json
...
Run /mahabharatha:status to monitor progress.
═══════════════════════════════════════════════════════════════
```
...
```
mahabharatha retry [OPTIONS] [TASK_IDS]...
...
cat ".mahabharatha/logs/worker-N.log" | grep "TASK-ID"

# Check verification command output
cat ".mahabharatha/logs/verification/TASK-ID.log"
```
