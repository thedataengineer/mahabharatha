...
def _build_docker_image() -> None:
    ...
    console.print("[cyan]Building mahabharatha-worker Docker image...[/cyan]")
    ...
    if result.returncode == 0:
        ...
        try:
            inspect = subprocess.run(
                ["docker", "image", "inspect", "--format", "{{.Size}}", "mahabharatha-worker"],
                capture_output=True,
                text=True,
                timeout=10,
            )
            if inspect.returncode == 0:
                ...
                console.print(f"[green]Image built: mahabharatha-worker ({size_mb:.0f} MB)[/green]")
            else:
...
def build(
    ...
    Examples:
        ...
        mahabharatha build --clean --watch

        mahabharatha build --dry-run
    """
    try:
        if docker:
            _build_docker_image()
