...
MERGE_BASE=$(git merge-base HEAD mahabharatha/$FEATURE/base)
NEW_FILES=$(git diff --name-only --diff-filter=A "$MERGE_BASE" HEAD -- '*.py' | grep -v 'tests/' | grep -v '__init__.py')

WIRING_WARNINGS=()
for FILE in $NEW_FILES; do
  # Convert path to module pattern (mahabharatha/foo.py -> mahabharatha.foo)
...
Wiring verification is advisory (warning only). Use `python -m mahabharatha.validate_commands --strict-wiring` to enforce.
