"""Integration tests for concurrent feature isolation via MAHABHARATHA_FEATURE env var.
...
class TestEnvVarPriority:
    """Tests that MAHABHARATHA_FEATURE env var takes priority over file-based detection."""

    def test_env_var_takes_priority_over_file(self, tmp_path: Path, monkeypatch) -> None:
        """MAHABHARATHA_FEATURE env var must win over .gsd/.current-feature file."""
        ...
        monkeypatch.setenv("MAHABHARATHA_FEATURE", "epic-1")
    ...
    def test_two_features_isolated_via_env_var(self, tmp_path: Path, monkeypatch) -> None:
        """Simulates two terminals with different MAHABHARATHA_FEATURE values.

        Each terminal sets a different MAHABHARATHA_FEATURE. Switching the env var
        ...
        monkeypatch.setenv("MAHABHARATHA_FEATURE", "epic-alpha")
        result_a = detect_feature()
        assert result_a == "epic-alpha"

        # Terminal B is working on epic-beta (same process, different env)
        monkeypatch.setenv("MAHABHARATHA_FEATURE", "epic-beta")
        result_b = detect_feature()
        assert result_b == "epic-beta"

        # Results must differ, proving isolation
        assert result_a != result_b, "Two different MAHABHARATHA_FEATURE values must produce different results"


class TestEnvVarFallthrough:
    """Tests that empty or unset MAHABHARATHA_FEATURE falls through to file detection."""

    def test_empty_env_var_falls_through_to_file(self, tmp_path: Path, monkeypatch) -> None:
        """Empty MAHABHARATHA_FEATURE must fall through to .gsd/.current-feature."""
        monkeypatch.chdir(tmp_path)
        ...
        assert result == "epic-from-file", f"Empty MAHABHARATHA_FEATURE should fall through to file, got '{result}'"

    def test_unset_env_var_falls_through_to_file(self, tmp_path: Path, monkeypatch) -> None:
        """Absent MAHABHARATHA_FEATURE must fall through to .gsd/.current-feature."""
        monkeypatch.chdir(tmp_path)
        ...
        monkeypatch.delenv("MAHABHARATHA_FEATURE", raising=False)

        result = detect_feature()

        assert result == "fallback-feature", f"Unset MAHABHARATHA_FEATURE should fall through to file, got '{result}'"


class TestEnvVarWhitespace:
    """Tests that whitespace in MAHABHARATHA_FEATURE is handled correctly."""

    def test_whitespace_env_var_stripped(self, tmp_path: Path, monkeypatch) -> None:
        """Leading/trailing whitespace in MAHABHARATHA_FEATURE must be stripped."""
        monkeypatch.chdir(tmp_path)

        monkeypatch.setenv("MAHABHARATHA_FEATURE", "  epic-spaces  ")

        result = detect_feature()

        assert result == "epic-spaces", f"Whitespace should be stripped from MAHABHARATHA_FEATURE, got '{result!r}'"
