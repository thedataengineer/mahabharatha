...
from mahabharatha.diagnostics.types import ErrorCategory, ErrorFingerprint, Evidence
        ...
        if category == ErrorCategory.CODE_ERROR:
            ...
            steps.append(
                RecoveryStep(
                    ...
                    command='find .mahabharatha/state -name "*.json" -exec python -m json.tool {} \\;',
                    risk="safe",
                    reversible=True,
                )
            )
            steps.append(
                RecoveryStep(
                    ...
                    command="cp .mahabharatha/state/*.json.bak .mahabharatha/state/",
                    risk="moderate",
                    reversible=True,
                )
            )
        ...
        if not steps:
            steps.append(
                RecoveryStep(
                    ...
                    command="mahabharatha debug --deep",
                    risk="safe",
                    reversible=True,
                )
            )
