...
# Mahabharatha Worker Execution (Details)

Reference material, examples, error recovery, and protocol specifications for Mahabharatha workers.
...
# Pull merged result
git fetch origin
git rebase origin/mahabharatha/FEATURE/staging
...
Workers write heartbeat files every 15 seconds to `.mahabharatha/state/heartbeat-{WORKER_ID}.json`:
...
Workers write progress to `.mahabharatha/state/progress-{WORKER_ID}.json`:
...
When a failure is ambiguous (unclear spec, missing dependency, unclear verification), workers escalate instead of silently blocking. Write to `.mahabharatha/state/escalations.json`:
...
Tier commands are configured in `.mahabharatha/config.yaml` under `verification_tiers`. If no tier 2 command is configured, the task's own `verification.command` is used.
...
2. **State File**: `.mahabharatha/state/{feature}.json` - supplementary shared state
3. **Progress Log**: `.gsd/specs/{feature}/progress.md` - human-readable log
4. **Worker Log**: `.mahabharatha/logs/worker-{id}.log` - detailed worker output
5. **Event Stream**: State manager appends events for orchestrator
6. **Heartbeat**: `.mahabharatha/state/heartbeat-{id}.json` - health monitoring
7. **Progress**: `.mahabharatha/state/progress-{id}.json` - structured progress
8. **Escalations**: `.mahabharatha/state/escalations.json` - shared escalation queue
