...
class Orchestrator:
    ...
    def get_status(self) -> dict:
        ...
        state_value = self._state.value if isinstance(self._state, OrchestratorState) else self._state
        return {
            "state": state_value,
    ...
    def complete_level(self, level: int) -> LevelMergeResult:
        ...
        worker_ids = [wt_id for wt_id, w in self._workers.items() if w.worktree is not None and w.state != "terminated"]

        # Map integer worker IDs to worktree worker IDs
        wt_worker_ids = [self._workers[wid].worktree.worker_id for wid in worker_ids if self._workers[wid].worktree]
        ...
        if not result.success:
            self._logger.error(f"Level {level} merge failed for worker {result.failed_worker}: {result.conflicts}")
