    ...
    def _check_file_ownership(self, level: int, tasks: list) -> CheckResult:
        ...
        for task in tasks:
            ...
            for f in created + modified:
                if f in file_owners:
                    issues.append(f"File conflict: {f} owned by both {file_owners[f]} and {task.id}")
                else:
    ...
    def _run_linter(self, files: list[str]) -> CheckResult:
        ...
        try:
            ...
            if not result.success:
                return CheckResult(passed=False, issues=[f"Lint failed: {result.stdout or result.stderr}"])
            return CheckResult(passed=True, issues=[])
