...
class TestRushCommand:
    ...
    def test_dry_run(self, tmp_path: Path, task_graph_file_setup: Path, monkeypatch: MonkeyPatch) -> None:
        ...
        monkeypatch.chdir(tmp_path)
        runner = CliRunner()
        with patch("mahabharatha.commands.kurukshetra.MahabharathaConfig") as mock_config_cls:
...
class TestRushExecution:
    ...
    def test_proceeds_on_confirm(
        ...
        runner = CliRunner()
        with (
            patch("mahabharatha.commands.kurukshetra.MahabharathaConfig") as mock_config_cls,
