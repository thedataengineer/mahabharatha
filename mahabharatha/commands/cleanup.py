...
def cleanup(
    ...
    Examples:
        ...
        mahabharatha cleanup --all --keep-logs --dry-run
...
def discover_features() -> list[str]:
    ...
    # Check state files
    state_dir = Path(".mahabharatha/state")
    if state_dir.exists():
        for state_file in state_dir.glob("*.json"):
            features.add(state_file.stem)

    # Check worktree directories
    worktree_dir = Path(".mahabharatha/worktrees")
    ...
    try:
        ...
        for branch in branches:
            if branch.name.startswith("mahabharatha/"):
    ...
    for feature in features:
        ...
        if not keep_branches:
            try:
                ...
                branches = git.list_branches()
                for branch in branches:
                    if branch.name.startswith(f"mahabharatha/{feature}/"):
        ...
        # Find containers
        plan["containers"].append(f"mahabharatha-worker-{feature}-*")

        # Find state files
        state_file = Path(f".mahabharatha/state/{feature}.json")
        if state_file.exists():
            plan["state_files"].append(str(state_file))

        # Find log files
        if not keep_logs:
            log_dir = Path(".mahabharatha/logs")
            if log_dir.exists():
