...
from mahabharatha.command_splitter import MIN_LINES_TO_SPLIT, CommandSplitter
...
STATE_PATTERNS: re.Pattern[str] = re.compile(r"state.*json|STATE_FILE|\.mahabharatha/state")
...
def validate_state_json_without_tasks(commands_dir: Path) -> tuple[bool, list[str]]:
    ...
    State JSON (.mahabharatha/state/) is supplementary to the Task ecosystem. Any
...
def validate_rules(rules_dir: Path | None = None) -> tuple[bool, list[str]]:
    ...
    Args:
        rules_dir: Path to rules directory. Defaults to .mahabharatha/rules/

    Returns:
        Tuple of (all_valid, list of error messages).
    """
    if rules_dir is None:
        rules_dir = Path(".mahabharatha/rules")
    ...
    try:
        from mahabharatha.rules import RuleLoader, RuleValidator
...
def validate_resilience_wiring(
    ...
    Args:
        package_dir: Path to the package directory. Defaults to mahabharatha/ (this package).
...
def validate_module_wiring(
    ...
    for candidate in candidates:
        ...
        # Top-level package name (e.g. "mahabharatha")
        pkg_name = package_dir.name

        # Build import patterns to search for
        # Full dotted path: mahabharatha.foo.bar
        full_dotted = f"{pkg_name}.{'.'.join(parts)}"
        # Module name (last component)
        module_name = parts[-1]
        # Parent dotted path: mahabharatha.foo (for "from mahabharatha.foo import bar")
        if len(parts) > 1:
...
class DocGenerator:
    ...
    def generate_command_doc(self, name: str) -> Path:
        ...
        title = f"/mahabharatha:{name}"
        for line in lines:
            if line.startswith("# "):
                # Extract command name from title like "# /mahabharatha:{name}"
                title = line[2:].strip()
                break
    ...
    def update_wiki_index(self, name: str, description: str) -> None:
        ...
        Adds line like:
          - [/mahabharatha:{name}](#mahabharatha{name}) — {description}
        ...
        anchor = f"mahabharatha{name.replace('-', '')}"
        entry = f"  - [/mahabharatha:{name}](#{anchor}) — {description}"

        # Check if entry already exists
        if f"/mahabharatha:{name}" in content:
            # Update existing entry
            pattern = rf"^\s*-\s*\[/mahabharatha:{re.escape(name)}\].*$"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to commands directory. Defaults to mahabharatha/data/commands/.",
    )
    parser.add_argument(
        "--strict-wiring",
        action="store_true",
