...
New modules added by `/zerg:design` task graphs may lack production callers, have unused config sections, or broken import chains. Currently, these issues are only caught by manual review or CI failures after the fact.
...
Add a mandatory **Wiring Verification** task to Level 5 (Quality) of every task graph generated by `/zerg:design`.
...
```json
{
  ...
  "verification": {
    "command": "python -m zerg.validate_commands && python -m pytest tests/ -x --timeout=60 -q",
    "timeout_seconds": 120
  },
  "estimate_minutes": 5,
  "skills_required": [],
...
- When `/zerg:design` generates `task-graph.json`, automatically append wiring verification task to L5
...
- Run `python -m zerg.validate_commands` for structural checks (module wiring, drift, imports)
...
- [ ] `/zerg:design` generates task graphs with wiring verification task in L5
...
- Security scanning (handled by separate `/zerg:analyze --check security`)
...
- `python -m zerg.validate_commands` (existing)
...
Modify `zerg/data/commands/design.core.md` Phase 3 to inject wiring task:
