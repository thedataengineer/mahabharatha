...
class ProjectCharter:
    ...
    created_by: str = "mahabharatha-inception"
    ...
    def from_dict(cls, data: dict[str, Any]) -> "ProjectCharter":
        ...
        return cls(
            ...
            created_by=data.get("created_by", "mahabharatha-inception"),
        )
...
def gather_requirements() -> ProjectCharter:
    ...
    charter.description = Prompt.ask(
        ...
        default="A new project created with Mahabharatha",
    )
...
def write_project_md(charter: ProjectCharter, output_dir: Path | None = None) -> Path:
    ...
    lines.extend(
        [
            "---",
            "",
            f"*Generated by Mahabharatha Inception Mode on {charter.created_at.strftime('%Y-%m-%d %H:%M')}*",
            "",
        ]
    )
