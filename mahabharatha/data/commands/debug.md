...
# Mahabharatha Debug: $ARGUMENTS

Deep diagnostic investigation for Mahabharatha execution issues with error intelligence, log correlation, Bayesian hypothesis testing, code-aware recovery, and environment diagnostics.
...
# Verify .mahabharatha/ directory exists
if [ ! -d ".mahabharatha" ]; then
  echo "WARNING: No .mahabharatha/ directory found. Limited diagnostics available."
fi

# If $ARGUMENTS is plain text (not a flag), treat as problem description
# e.g., /mahabharatha:debug workers keep crashing
```
...
1. **Phase 1: Context Gathering** — Read Mahabharatha state, logs, task-graph, design doc, and git state in parallel. Output a CONTEXT SNAPSHOT.
...
9. **Phase 6.5: Design Escalation Check** — If issues require architectural changes, recommend `/mahabharatha:design`.
...
```
/mahabharatha:debug — Deep diagnostic investigation for Mahabharatha execution issues.
