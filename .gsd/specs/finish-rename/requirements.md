...
The z-shortcut-and-rename feature was partially completed. All `mahabharatha/` package code (commands, diagnostics, CLI, tests) was renamed. Three areas remain:

1. `.mahabharatha/` runtime scripts (troubleshoot.py, test_troubleshoot.py)
2. Installed commands in `.claude/commands/` (stale `mahabharatha:troubleshoot.md`)
3. Documentation/backlog references to old paths

## Functional Requirements

### FR-1: Rename .mahabharatha/ runtime scripts
- `.mahabharatha/troubleshoot.py` → `.mahabharatha/debug.py` with all class/docstring renames
- `.mahabharatha/tests/test_troubleshoot.py` → `.mahabharatha/tests/test_debug.py` with import updates

### FR-2: Fix installed commands
- Reinstall commands so `.claude/commands/mahabharatha:debug.md` replaces `mahabharatha:troubleshoot.md`
...
- `mahabharatha/data/commands/mahabharatha:debug.md` — fix one stray "troubleshooting" ref
