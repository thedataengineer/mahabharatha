...
def mock_orchestrator_deps() -> Generator[dict[str, MagicMock], None, None]:
    ...
    with (
        ...
        patch("mahabharatha.orchestrator.MetricsCollector") as metrics_mock,
    ):
        # StateManager mock
        state = MagicMock()
...
def container_mode_config() -> ZergConfig:
    ...
    return ZergConfig(
        ...
        logging=LoggingConfig(
            level="debug",
            directory="/tmp/mahabharatha-logs",
        ),
    )
...
def devcontainer_setup(tmp_path: Path) -> Path:
    ...
    (tmp_path / ".mahabharatha" / "state").mkdir()
...
def container_launcher_config() -> LauncherConfig:
    ...
    return LauncherConfig(
        ...
        log_dir=Path("/tmp/mahabharatha-logs"),
    )
...
def orchestrator_factory(
    ...
    def _create_orchestrator(
        ...
        (tmp_path / ".mahabharatha").mkdir(exist_ok=True)
