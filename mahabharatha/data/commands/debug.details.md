...
# Mahabharatha Debug: Details & Reference

Full templates, evidence checklists, examples, and edge cases for `mahabharatha:debug`.

---

## Phase 1: Context Gathering — Templates

**Mahabharatha State** (read in parallel — TaskList is authoritative, state JSON is supplementary):
- TaskList / TaskGet — authoritative task status
- `.mahabharatha/state/<feature>.json` - Task states, worker states, errors (fallback)
- `.mahabharatha/logs/worker-*.stderr.log` - Last 50 lines from each worker
...
- [ ] Check worktree exists: `.mahabharatha/worktrees/worker-<id>/`
- [ ] Check worker branch: `git -C .mahabharatha/worktrees/worker-<id> log -1`
...
- [ ] Check `.mahabharatha/config.yaml` for misconfigurations
...
- **Prior probabilities** are drawn from a knowledge base of 30+ known failure patterns spanning Python, JavaScript, Go, Rust, Docker, Git, and Mahabharatha-specific issues. Each pattern carries a calibrated prior probability.
...
After generating the recovery plan, check if the diagnosed issues require architectural changes that go beyond tactical fixes. If so, recommend `/mahabharatha:design` to the user.
...
Action:  Run /mahabharatha:design to create a new architecture
         or 'mahabharatha design' from the CLI

Note: Tactical recovery steps above will stabilize the current state.
      Design addresses the underlying structural issue.
```

**Behavior with `--fix`**: When `--fix` is active and design escalation is detected, execute the tactical recovery steps first (stabilize), then display the design escalation recommendation. Do NOT auto-invoke `/mahabharatha:design` — it requires human approval.
...
- Available images relevant to Mahabharatha
...
- Parse and validate `.mahabharatha/config.yaml` against expected schema
...
1. Check `.mahabharatha/config.yaml` exists
...
2. Check `.mahabharatha/state/<feature>.json.bak`
...
2. Remove `.mahabharatha/worktrees/*/`
