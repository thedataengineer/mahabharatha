...
class RushCommand:
    ...
    def execute(
        ...
        return ExecutionResult(
            validated=True,
            execution_id=state.feature,
            tasks_completed=len([t for t in graph.tasks.values() if getattr(t, "status", None) == "complete"]),
            tasks_failed=len([t for t in graph.tasks.values() if getattr(t, "status", None) == "failed"]),
        )
