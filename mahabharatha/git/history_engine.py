...
from mahabharatha.git.commit_engine import COMMIT_TYPE_PATTERNS
...
class SafeRewriter:
    ...
    def execute_squash(self, plan: list[dict[str, Any]], base: str = "main") -> bool:
        ...
        try:
            fd, script_path = tempfile.mkstemp(suffix=".py", prefix="mahabharatha_rebase_")
            os.fchmod(fd, 0o700)
    ...
    def execute_reorder(self, ordered_shas: list[str], base: str = "main") -> bool:
        ...
        try:
            fd, script_path = tempfile.mkstemp(suffix=".py", prefix="mahabharatha_reorder_")
            os.fchmod(fd, 0o700)
            with os.fdopen(fd, "w") as script_file:
                script_file.write("#!/usr/bin/env python3\n")
                script_file.write("import sys\n")
