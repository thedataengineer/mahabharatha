<!-- SPLIT: details, parent: Kurukshetra.md -->
# Kurukshetra — Detailed Reference

This file contains extended examples, templates, and edge cases.
Core instructions are in `Kurukshetra.core.md`.
...
├──────────┼────────┼────────────────────────────┼──────────┤
│ worker-0 │ 49152  │ mahabharatha/{feature}/worker-0 │ Starting │
│ worker-1 │ 49153  │ mahabharatha/{feature}/worker-1 │ Starting │
│ worker-2 │ 49154  │ mahabharatha/{feature}/worker-2 │ Starting │
│ worker-3 │ 49155  │ mahabharatha/{feature}/worker-3 │ Starting │
│ worker-4 │ 49156  │ mahabharatha/{feature}/worker-4 │ Starting │
└──────────┴────────┴────────────────────────────┴──────────┘

Monitor:  mahabharatha status --dashboard  (run in separate terminal)

───────────────────────────────────────────────────────────────

Commands (after Kurukshetra completes):
  /mahabharatha:status     - Check progress snapshot
  /mahabharatha:logs N     - Stream logs from worker N
...
```
mahabharatha Kurukshetra [OPTIONS]
...
```bash
mahabharatha Kurukshetra --mode auto
```
...
```bash
mahabharatha Kurukshetra --mode subprocess
```
...
- Creates Docker network `mahabharatha-internal` for worker isolation
- Mounts worktrees as bind volumes
- Passes ANTHROPIC_API_KEY to containers
- Executes `.mahabharatha/worker_entry.sh` in each container

### Task Tool Mode (Slash Command)

```bash
mahabharatha Kurukshetra --mode task
```

- Used automatically when `/mahabharatha:Kurukshetra` is invoked as a Claude Code slash command
...
```
You are Mahabharatha Worker executing task {TASK_ID} for feature "{FEATURE}".
...
| Resume | State file checkpoint | Re-run `/mahabharatha:Kurukshetra --resume` |
...
mahabharatha Kurukshetra --resume --level 3
```

Resume behavior:
1. Calls TaskList to identify completed/pending tasks (authoritative)
2. Cross-references `.mahabharatha/state/{feature}.json` for supplementary state
  ...
  "workers": [
    {
      ...
      "worktree": ".mahabharatha/worktrees/user-auth-worker-0",
      "port": 49152,
      "container_id": "abc123",
      "status": "running",
      "assignments": {
