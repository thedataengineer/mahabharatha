...
**Feature**: Pre-release preparation for MAHABHARATHA public release as `mahabharatha-ai` on PyPI
**Epic**: #179 | **Issues**: #180, #181, #182, #183, #184
**Status**: APPROVED

## Problem Statement

MAHABHARATHA is ready for public release but the PyPI package name `mahabharatha` is taken. The package must be renamed to `mahabharatha-ai`, the CHANGELOG frozen, WIP files cleaned, GitHub infrastructure configured, and the release pipeline validated end-to-end via TestPyPI.

## Scope

### In Scope
- Rename PyPI distribution name from `mahabharatha` to `mahabharatha-ai` (issues #180)
...
|----------|--------|-----------|
| PyPI name | `mahabharatha-ai` | `mahabharatha` taken; brandable, search-friendly |
...
2. `pip install mahabharatha-ai` works from TestPyPI
3. `mahabharatha --help` works after install
4. No stale `pip install mahabharatha` refs (only `mahabharatha-ai`)
