...
# Mahabharatha Estimate — Details

Reference material for `/mahabharatha:estimate`. See `estimate.core.md` for workflow and flags.
...
import sys, json
sys.path.insert(0, '.')
sys.path.insert(0, '.mahabharatha')
...
```json
{
  "schema": "mahabharatha-estimate-v1",
...
```json
{
  "schema": "mahabharatha-calibration-v1",
  "bias_factor": 1.15,
  "per_grade": {
    "A": 1.05,
    "B": 1.12,
...
### No task-graph.json
- Pre mode: error with "Run /mahabharatha:design first to generate task graph"
- Post/Calibrate: proceed without pre-estimate, compare against raw state data

### No state file (post mode)
- Error with "No execution data found. Run /mahabharatha:Kurukshetra first"

### No history files (calibrate mode)
- Warning: "No completed features with both pre and post estimates found"
- Suggest: "Run /mahabharatha:estimate --pre before Kurukshetra, then --post after, to build history"
...
- Log warning: "Could not import mahabharatha modules — using manual computation"
