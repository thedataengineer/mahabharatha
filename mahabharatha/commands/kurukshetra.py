...
from mahabharatha.backlog import update_backlog_task_status
from mahabharatha.capability_resolver import CapabilityResolver
from mahabharatha.config import MahabharathaConfig
from mahabharatha.logging import get_logger, setup_logging
...
def kurukshetra(
    ...
    try:
        ...
        if not task_graph_path or not task_graph_path.exists():
            ...
            console.print("Run [cyan]mahabharatha design[/cyan] first to create a task graph")
            raise SystemExit(1)
...
def _run_preflight(config: MahabharathaConfig, mode: str, workers: int) -> bool:
